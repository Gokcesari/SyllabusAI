...
public class AuthController : ControllerBase
{
    ...
    public async Task<IActionResult> Login([FromBody] LoginRequest request, CancellationToken cancellationToken)
    {
        ...
        var result = await _authService.LoginAsync(request, webPortalClient, cancellationToken);
        ...
        if (result.ErrorCode == "AdminWebLoginForbidden")
        {
            _logger.LogWarning("Admin web giriş denemesi reddedildi: {Email}", request.Email);
            return StatusCode(403, new { message = result.ErrorMessage });
        }
